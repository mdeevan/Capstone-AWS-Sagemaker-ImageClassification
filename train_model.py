...
def test(model, test_dataloader, criterion, hook, device=torch.device("cpu")):
    '''
    ...
    logger.info('model training: model test starting')
    ...
    with torch.no_grad():
        ...
        logger.info("model training: Test loss : {:.3f},\
            Test set: Accuracy: {:.3f} ".format(test_loss,
                                        accuracy
                                        ))


        logger.info("model training: model testing completed")

def train(model, train_dataloader, valid_dataloader, criterion, optimizer, hook, epochs=2, device=torch.device("cpu")):
    '''
    ...
    logger.info('model training: Training started on device {}'.format(device))
...
def main(args):
    ...
    logger.info('model training: create the data loaders')
    ...
    if hook:
        logger.info('model training: hook loss criterion')
    ...
    logger.info('model training: train the model')
    model=    train(model, train_loader, valid_loader, loss_criterion, optimizer, hook, args.epochs, device)


    '''
    TODO: Test the model to see its accuracy
    '''
    logger.info("model training: Test the Model")
    ...
    logger.info("model training: Script competed, exiting")
